...
class FacultyController extends Controller
{
    public function home(){
       $approve = ReservationEvent::where('status_id',1)->where('user_id',Auth::user()->id)->count(); 
    	...
    	return view('faculty.home', compact('calendars','approve','pending'));
    }
    ...
    public function faculty_list(){
    	...
    	return view('faculty.list', compact('datas'));
    }

    public function faculty_set(){
        $calendars = ReservationEvent::where('status_id',1)->orWhere('status_id',3)->get();
        $departments = Department::all();
        $times = Time::all();
    	return view('faculty.set',compact('calendars','departments','times'));
    }

    public function faculty_get_rooms(Request $request){
        $rooms = Room::where('department_id', $request['bldgId'])->get();
        return response()->json($rooms);
    }

    public function faculty_insert_event(Request $request){
        ...
        return redirect()->route('faculty_review_details',$res->id)->with('suc', 'You have Successfully Added an Event');
    }

    public function faculty_review_details($id){
        $find = ReservationEvent::findOrFail($id);
        $calendars = ReservationEvent::where('status_id',1)->orWhere('status_id',3)->get();
        return view('faculty.review-details', compact('find','calendars'));
    }

    public function faculty_approve_details($id){
        $reserve = ReservationEvent::findOrFail($id);
        $reserve->status_id = 2;
        $reserve->save();
        return redirect()->route('faculty_set')->with('suc', 'You have Successfully Added an Event');
    }
}
